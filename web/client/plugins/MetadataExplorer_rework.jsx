...
const {addService, textSearch, changeCatalogFormat, changeCatalogMode,
    changeNewUrl, changeNewTitle, changeNewType, changeSelectedService, addLayer, addLayerError, resetCatalog} = require("../actions/catalog");
...
const catalogSelector = createSelector([
    ...
    (state) => get(state, `catalog.newService`),
    (state) => get(state, `catalog.newService.type`, 'csw'),
    (state) => get(state, `catalog.services.${get(state, "catalog.selectedService")}.type`, 'csw'),
    (state) => state && state.catalog && state.catalog.searchOptions
], (result, newService, newformat, selectedFormat, options) =>({
    format: newformat,
    newService,
...
})(require('../components/catalog/Catalog_rework'));
