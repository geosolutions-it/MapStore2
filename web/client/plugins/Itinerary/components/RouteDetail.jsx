...
import { formatDistance, formatTime, getRouteId, getSignIcon } from '../utils/ItineraryUtils';
...
const RouteDetail = ({ itineraryData, onAddAsLayer }) => {
    ...
    const getFeaturesByIndex = (index) => {
        return features.filter(feature => get(feature, 'id', '').endsWith(`-${index}`));
    };
    ...
    const onAddLayer = (index) => {
        onAddAsLayer({
            ...
            style: style ? {
                ...
                body: {
                    ...style?.body,
                    rules: get(style, 'body.rules', [])
                        .filter(rule => rule.id === getRouteId(index))
                }
            } : {}
        });
    };
