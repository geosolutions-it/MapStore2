...
describe('DynamicLegend Plugin', () => {
    ...
    it('Shows DynamicLegend plugin', () => {
        const { Plugin } = getPluginForTest(DynamicLegend, {
            controls: {
                addgroup: {
                    enabled: true
                }
    ...
    it('DynamicLegend shows annotations layer in openlayers mapType', () => {
        const { Plugin } = getPluginForTest(DynamicLegend, {
            layers: {
    ...
    it('DynamicLegend hides filter layer if no groups and no layers are present', () => {
        const { Plugin } = getPluginForTest(DynamicLegend, {
            layers: {
    ...
    it('DynamicLegend hides filter layer if a group with no layers are present', () => {
        const { Plugin } = getPluginForTest(DynamicLegend, {
            layers: {
                groups: [],
                flat: []
    ...
    it('DynamicLegend use custom group node', () => {
        const { Plugin } = getPluginForTest(DynamicLegend, {
            layers: {
                groups: [{
                    expanded: true,
    ...
    it('DynamicLegend use custom layer node', () => {
        const { Plugin } = getPluginForTest(DynamicLegend, {
            layers: {
                groups: [{
                    expanded: true,
    ...
    describe('render items from other plugins', () => {
        ...
        describe('target: toolbar', () => {
            ...
            it('selector reads status, selectedGroups, selectedLayers', () => {
                const { Plugin } = getPluginForTest(DynamicLegend, SELECTED_LAYER_STATE);
                const WrappedPlugin = dndContext(Plugin);
                ReactDOM.render(<WrappedPlugin items={[{
                    name: "Custom",
                    target: "toolbar",
        ...
        it('render WidgetsBuilder', () => {
            const { Plugin } = getPluginForTest(DynamicLegend, { ...SELECTED_LAYER_STATE, controls: { widgetBuilder: {available: true}}});
            const WrappedPlugin = dndContext(Plugin);
