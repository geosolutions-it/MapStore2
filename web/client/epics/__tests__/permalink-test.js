...
describe('Permalink Epics', () => {
    ...
    it('test loadPermalinkEpic does not evaluate the content of the placeholders', (done) => {
        window.__permalinkEvaluated = undefined;
        setApiGetResource(api, {
            getResource: () => Rx.Observable.of({
                name: "test",
                attributes: { type: "map", pathTemplate: "/viewer/${(window.__permalinkEvaluated = true)}" }
            })
        });
        const NUMBER_OF_ACTIONS = 2;
        testEpic(
            ...
            NUMBER_OF_ACTIONS, [
                ...
                expect(actions.length).toBe(NUMBER_OF_ACTIONS);
                expect(window.__permalinkEvaluated).toBe(undefined);
                done();
            }, state);
    });
