...
import { searchEpic, searchItemSelected, textSearchShowGFIEpic, zoomAndAddPointEpic, searchOnStartEpic } from '../search';
...
describe('search Epics', () => {
    ...
    it('produces the selectSearchItem epic and GFI for single layer', (done) => {
        ...
        testEpic(addTimeoutEpic(searchItemSelected, 100), NUM_ACTIONS, action, (actions) => {
            ...
            expect(actions[1].overrideParams).toEqual({"gs:layername": {info_format: "text/html"}});
    ...
    it('searchItemSelected epic with a service with openFeatureInfoButtonEnabled=false', (done) => {
        ...
        testEpic(addTimeoutEpic(searchItemSelected, 100), NUM_ACTIONS, action, (actions) => {
            ...
            let expectedActions = [TEXT_SEARCH_RESULTS_PURGE, FEATURE_INFO_CLICK, SHOW_MAPINFO_MARKER, ZOOM_TO_EXTENT, TEXT_SEARCH_ADD_MARKER, TEST_TIMEOUT];
            let actionsType = actions.map(a => a.type);
