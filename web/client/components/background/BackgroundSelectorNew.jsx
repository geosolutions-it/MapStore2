...
function MetadataExplorerAdd({onClickHandler = () => {}}) {
    return (
...
function BackgroundSelector({
    ...
    const configuredItemsBackgroundTools = [{ name: 'MetadataExplorer', Component: showAddBtnForBgLayer && MetadataExplorerAdd, onClickHandler: () => {
            ...
            <FlexBox column gap="xs" classNames={['ms-background-selector', '_absolute', '_corner-bl']} style={style}>
                {open || alwaysVisible ? <FlexBox component="ul" gap="md" column classNames={['ms-background-selector-list', 'ms-main-colors', '_padding-tb-sm', 'shadow']}>
                    <BackgroundLayersList
                        ...
                        tools={<>
                            {configuredItemsBackgroundTools.map(({ name, Component, onClickHandler }) => Component && <Component key={name} onClickHandler={onClickHandler} />)}
                        </>}
                    />
                    {terrains.length && isCesium ? <BackgroundLayersList
                        ...
                        tools={
                            <>
                                {showAddBtnForBgLayer && <button onClick={() => setShowTerrainModal({open: true})}>
                                    <Glyphicon glyph="plus" />
                                </button>}
                            </>
                        }
