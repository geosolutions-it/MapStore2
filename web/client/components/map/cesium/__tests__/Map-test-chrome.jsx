...
describe("CesiumMap", () => {
    ...
    it('check if the handler for "moveend" event is called', (done) => {
        ...
        cesiumMap.camera.moveEnd.addEventListener(() => {
            ...
            // check camera moved
            expect(
                Math.round(spy.calls[0].arguments[0].y * precision) / precision
            ).toBe(30);
            expect(
                Math.round(spy.calls[0].arguments[0].x * precision) / precision
            ).toBe(20);
        ...
        cesiumMap.camera.setView({
            destination: Cesium.Cartesian3.fromDegrees(20, 30, 5000000),
            orientation: {
                heading: 1,
                pitch: (-1 * Math.PI) / 2,
                roll: 0
    ...
    it("check if the map changes when receive new props", () => {
        let map = ReactDOM.render(
            <CesiumMap center={{ y: 43.9, x: 10.3 }} zoom={10} />,
            document.getElementById("container")
        );
        ...
        map = ReactDOM.render(
            <CesiumMap center={{ y: 44, x: 10 }} zoom={12} />,
            document.getElementById("container")
        );

        expect(
            Math.round(
                cesiumMap.camera.positionCartographic.height -
                    map.getHeightFromZoom(12)
            )
        ).toBe(0);
        expect(
            Math.round(
                (cesiumMap.camera.positionCartographic.latitude * 180.0) /
                    Math.PI
            )
        ).toBe(44);
        expect(
            Math.round(
                (cesiumMap.camera.positionCartographic.longitude * 180.0) /
                    Math.PI
            )
        ).toBe(10);
    });
    ...
    describe("hookRegister", () => {
        ...
        it("with custom hookRegister", () => {
            ...
            const map = ReactDOM.render(
                <CesiumMap
                    hookRegister={customHooRegister}
                    id="mymap"
                    center={{ y: 43.9, x: 10.3 }}
                    zoom={11}
                />,
