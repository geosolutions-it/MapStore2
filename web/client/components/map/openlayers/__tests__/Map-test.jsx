...
describe('OpenlayersMap', () => {
    ...
    it('click on feature preserves clicked point', (done) => {
        ...
        setTimeout(() => {
            ...
            expect(spy.mock.calls.length).toEqual(1);
            expect(spy.mock.calls[0][0].latlng.lat).toBe(0.5);
            expect(spy.mock.calls[0][0].latlng.lng).toBe(0.5);
            done();
        }, 500);
    });
    ...
    it('click on layer with handleClickOnLayer with marker change clicked point coordinate', (done) => {
        ...
        setTimeout(() => {
            ...
            expect(spy.mock.calls.length).toEqual(1);
            // check the point
            expect(spy.mock.calls[0][0].latlng.lat).toBe(10);
            expect(spy.mock.calls[0][0].latlng.lng).toBe(43);
            // also layer id is passed (e.g. used as flag for hide GFI marker)
            expect(spy.mock.calls[0][1]).toBe("ID");
    ...
    it('check if the handler for "moveend" event is called after setZoom', (done) => {
        ...
        olMap.on('moveend', () => {
            ...
            expect(spy.mock.calls[1].length).toEqual(6);
            expect(normalizeFloat(spy.mock.calls[1][0].y, 1)).toBe(43.9);
            expect(normalizeFloat(spy.mock.calls[1][0].x, 1)).toBe(10.3);
            expect(spy.mock.calls[1][1]).toBe(12);
            expect(spy.mock.calls[1][2].bounds).toBeTruthy();
            expect(spy.mock.calls[1][2].crs).toBeTruthy();
            expect(spy.mock.calls[1][3].height).toBeTruthy();
            expect(spy.mock.calls[1][3].width).toBeTruthy();
            done();
        });
    ...
    it('check if the handler for "moveend" event is called after setCenter', (done) => {
        ...
        olMap.on('moveend', () => {
            ...
            expect(spy.mock.calls.length).toEqual(2);
            expect(spy.mock.calls[1].length).toEqual(6);
            expect(normalizeFloat(spy.mock.calls[1][0].y, 1)).toBe(44);
            expect(normalizeFloat(spy.mock.calls[1][0].x, 1)).toBe(10);
            expect(spy.mock.calls[1][1]).toBe(11);
            expect(spy.mock.calls[1][2].bounds).toBeTruthy();
            expect(spy.mock.calls[1][2].crs).toBeTruthy();
            expect(spy.mock.calls[1][3].height).toBeTruthy();
            expect(spy.mock.calls[1][3].width).toBeTruthy();
    ...
    it('check if the map reprojects the view coordinates when the projection is changed', () => {
        ...
        expect(firstView[0].toFixed(0)).not.toEqual(secondView[0].toFixed(0));
    ...
    it('test ZOOM_TO_EXTENT_HOOK', (done) => {
        ...
        olMap.on('moveend', () => {
            ...
            if (spy.mock.calls[2]) {
                ...
                expect(spy.mock.calls[2][0].x).toBeGreaterThan(9);
                expect(spy.mock.calls[2][0].y).toBeGreaterThan(9);
                expect(spy.mock.calls[2][0].x).toBeLessThan(11);
                expect(spy.mock.calls[2][0].y).toBeLessThan(11);
        ...
        expect(spy.mock.calls[0]).toBeTruthy(); // first is called by initial render
    ...
    it('test ZOOM_TO_EXTENT_HOOK with full extent', (done) => {
        ...
        olMap.on('moveend', () => {
            expect(spy.mock.calls[1]).toBeTruthy();
            expect(spy.mock.calls[1][0].x).toEqual(0);
            expect(spy.mock.calls[1][0].y).toEqual(0);
            // Bbox should be max.
            expect(spy.mock.calls[1][1]).toEqual(0);
            expect(spy.mock.calls[1][2].bounds.maxx).toBeGreaterThan(15654300);
            expect(spy.mock.calls[1][2].bounds.maxy).toBeGreaterThan(7827150);
            expect(spy.mock.calls[1][2].bounds.minx).toBeLessThan(-15654300);
            expect(spy.mock.calls[1][2].bounds.miny).toBeLessThan(-7827150);
            done();
    ...
    it('test ZOOM_TO_EXTENT_HOOK with full extent', (done) => {
        ...
        olMap.on('moveend', () => {
            expect(spy.mock.calls[1]).toBeTruthy();
            expect(spy.mock.calls[1][0].x).toBeGreaterThan(0);
            expect(spy.mock.calls[1][0].y).toBeGreaterThan(0);
            expect(spy.mock.calls[1][0].x).toBeLessThan(2);
            expect(spy.mock.calls[1][0].y).toBeLessThan(2);
            // Bbox should not be max.
            expect(spy.mock.calls[1][1]).toBe(21);
            expect(spy.mock.calls[1][2].bounds.maxx).toBeGreaterThan(111319);
            expect(spy.mock.calls[1][2].bounds.maxy).toBeGreaterThan(111325);
            expect(spy.mock.calls[1][2].bounds.minx).toBeLessThan(111320);
            expect(spy.mock.calls[1][2].bounds.miny).toBeLessThan(111326);
            done();
    ...
    it('test updateMapInfoState with custom projection 3003 (not listed in wrappedProjections)', () => {
        ...
        expect(spyOnMapViewChanges.mock.calls.length).toBe(0);
    });
    ...
    it('test updateMapInfoState projection 3857', () => {
        ...
        expect(spyOnMapViewChanges.mock.calls.length).toBe(1);
    });
