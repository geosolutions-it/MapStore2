...
describe('Test DrawSupport', () => {
    ...
    it('drawsupport test for replacePolygonsWithCircles', () => {
        const fakeMap = {
            addLayer: () => {},
            removeLayer: () => {},
            disableEventListener: () => {},
        ...
        expect(support).toExist();
        const olFeature = new ol.format.GeoJSON().readFeatures(CIRCLE)[0];
        const geoms = support.replacePolygonsWithCircles(olFeature);
        geoms.forEach(g => {
            expect(g.getType() === "Circle").toBe(true);
        });

    });
    it('drawsupport test for replaceCirclesWithPolygons', () => {
        const fakeMap = {
            addLayer: () => {},
            removeLayer: () => {},
            disableEventListener: () => {},
        ...
        expect(support).toExist();
        const olFeature = new ol.format.GeoJSON().readFeatures(CIRCLE)[0];
        const geoms = support.replaceCirclesWithPolygons(olFeature);
        geoms.forEach(g => {
            expect(g.getType() === "Polygon").toBe(true);
        });
