...
describe("test the OL GrabMap component", () => {
    beforeEach((done) => {
        document.body.innerHTML = '<div id="container"></div><div id="map"><canvas></canvas></div>';
        setTimeout(done);
    });
    ...
    it('test component snapshot img creation', (done) => {
        ...
        const tb = ReactDOM.render(<GrabMap config={map} layers={layers} snapstate={{state: "DISABLED"}} active={false} timeout={0} onSnapshotReady={() => { done(); }}/>, document.getElementById("container"));
        expect(tb).toExist();
