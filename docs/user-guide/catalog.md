...
* *Use remote custom tile grids*: allows use of the layer cached tiles. If checked, the *Tiled=true* URL parameter will be added to the WMS request to [use tiles cached with GeoWebCache](https://docs.geoserver.org/latest/en/user/geowebcache/using.html#direct-integration-with-geoserver-wms), these will be automatically applied to the [Layer Settings](layer-settings.md#display) when a layer is added to the map from this source.
