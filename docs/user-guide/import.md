...
* *Map context files* (supported formats: MapStore legacy format and WMC)
...
A map context is, for example, the file that an user download selecting the <img src="../img/button/export.jpg" class="ms-docbutton"/> option in [Burger Menu](menu-bar.md#burger-menu). There are two types of file that can be exported:

* The <img src="../img/button/mapstore_format_button.jpg" class="ms-docbutton"/> file, is a `json` format contains all map context information, that means the settings about projections, coordinates, zoom, extent and layers present in the map (more information can be found in the [Maps Configuration](../developer-guide/maps-configuration.md) section of the [Developer Guide](https://mapstore.readthedocs.io/en/latest/developer-guide/).
...
* The <img src="../img/button/wmc_format_button.jpg" class="ms-docbutton"/> (Web Map Context) file, is a `xml` format that contains only the layers present in the map and keeping their settings related to projections, coordinates, zoom and extension (more information can be found in the  [Maps Configuration](../developer-guide/maps-configuration.md#web-map-context) section of the [Developer Guide](https://mapstore.readthedocs.io/en/latest/developer-guide/).
