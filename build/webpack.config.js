...
const config = require("./buildConfig")({
    bundles: {
        [process.env.bundle || "mapstore2"]: path.join(
            __dirname,
            "..",
            "web",
            "client",
            "product",
            process.env.entrypoint || process.env.bundle || "app"
        ),
        embedded: path.join(
            __dirname,
            "..",
    ...
    proxy: {
        "/rest": {
            target: "http://localhost/mapstore",
        },
        "/pdf": {
            target: "http://localhost/mapstore",
        },
